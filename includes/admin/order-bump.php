...
class VICUFFW_CHECKOUT_UPSELL_FUNNEL_Admin_Order_Bump {
                                            ...
                                            <div class="field">
                                                ...
                                                <select class="vi-wcuf-search-select2 vi-wcuf-search-product vi-wcuf-pd-condition-ob_product vi-wcuf-condition-value"
													...
													if ( $ob_product && ( $product = wc_get_product( $ob_product ) ) ) {
														/* translators: %1s: order bump product id, %2s: order bump product name */
														echo sprintf( '<option value="%1s" selected>%2s</option>', esc_attr( $ob_product ), wp_kses_post( $product->get_formatted_name() ) );
													}
													?>
                                                </select>
                                                <p class="description">
	...
	public function admin_enqueue_scripts() {
		if ( isset( $_REQUEST['_viwcuf_settings_ob'] ) && ! wp_verify_nonce( wc_clean($_REQUEST['_viwcuf_settings_ob']), '_viwcuf_settings_ob_action' ) ) {
			return;
		}
