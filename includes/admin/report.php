...
class VICUFFW_CHECKOUT_UPSELL_FUNNEL_Admin_Report {
	...
	public function __construct() {
		$this->start_date = gmdate( 'Y-m-d', strtotime( '-30 days' ) );
		$this->end_date   = gmdate( 'Y-m-d' );
	...
	public function settings_callback() {
		if ( isset( $_REQUEST['_viwcuf_report'] ) && ! wp_verify_nonce( wc_clean($_REQUEST['_viwcuf_report']), '_viwcuf_report_action' ) ) {
			return [];
		}
        ...
        <div class="wrap">
            ...
            <form class="vi-ui form" method="post">
				...
				if ( $product_id && $type === 'products' ) {
					echo sprintf( '<div class="thirteen wide field viwcuf-product-title">%s</div>', wp_kses_post( get_the_title( $product_id ) ) );
				}
				?>
