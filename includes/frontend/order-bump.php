...
class VICUFFW_CHECKOUT_UPSELL_FUNNEL_Frontend_Order_Bump {
	...
	public function viwcuf_ob_woocommerce_update_order_review_fragments( $result ) {
		...
		if ( self::$rules ) {
			...
			foreach ( self::$rules as $rule_id ) {
				$sc_rule_id = $rule_id;
				if ( $rule_id && count( shortcode_parse_atts( $rule_id ) ) > 1 ){
					$sc_rule_id = '';
				}
				$shortcode_html = do_shortcode( '[viwcuf_checkout_order_bump id="' . $sc_rule_id . '" ]' );
				if ( ! $shortcode_html ) {
					continue;
				}
	...
	public function viwcuf_wp_enqueue_scripts() {
		...
		wp_enqueue_script( 'viwcuf-frontend-order', VICUFFW_CHECKOUT_UPSELL_FUNNEL_JS . 'frontend-order.' . $suffix . 'js', array( 'jquery' ), VICUFFW_CHECKOUT_UPSELL_FUNNEL_VERSION, false );
