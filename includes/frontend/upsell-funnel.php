...
class VICUFFW_CHECKOUT_UPSELL_FUNNEL_Frontend_Upsell_Funnel {
	...
	public function viwcuf_wc_get_template( $located, $template_name ) {
		if ( is_wc_endpoint_url( 'viwcuf_us_endpoint' ) && $template_name === 'checkout/form-checkout.php' ) {
			...
			$sc_product_ids = $product_ids;
			if ( $sc_rule && count( shortcode_parse_atts( $sc_rule ) ) > 1 ){
				$sc_rule = '';
			}
			if ( $sc_position && count( shortcode_parse_atts( $sc_position ) ) > 1 ){
				$sc_position = '';
			}
			if ( $sc_product_ids && count( shortcode_parse_atts( $sc_product_ids ) ) > 1 ){
				$sc_product_ids = '';
			}
			$shortcode_html = do_shortcode( '[viwcuf_checkout_upsell_funnel rule="' . $sc_rule . '" position="' . $sc_position . '" product_ids="' . esc_attr( $sc_product_ids ) . '"]' );
	...
	public function viwcuf_wp_enqueue_scripts() {
		...
		wp_enqueue_script( 'viwcuf-frontend-upsell', VICUFFW_CHECKOUT_UPSELL_FUNNEL_JS . 'frontend-upsell.' . $suffix . 'js', array( 'jquery', 'wc-checkout' ), VICUFFW_CHECKOUT_UPSELL_FUNNEL_VERSION, false );
		wp_enqueue_script( 'viwcuf-frontend-swatches', VICUFFW_CHECKOUT_UPSELL_FUNNEL_JS . 'frontend-swatches.' . $suffix . 'js', array( 'jquery' ), VICUFFW_CHECKOUT_UPSELL_FUNNEL_VERSION, false );
		if ( $display_type === 'slider' ) {
			wp_enqueue_script( 'viwcuf-flexslider', VICUFFW_CHECKOUT_UPSELL_FUNNEL_JS . 'vi-flexslider.min.js', array( 'jquery' ), VICUFFW_CHECKOUT_UPSELL_FUNNEL_VERSION, false );
	...
	public static function get_us_product_ids( $rule, $settings ) {
		...
		if ( ! empty( $found_pd_ids ) && empty( $product_ids ) ) {
			...
			switch ( $product_order_by ) {
				...
				case 'price':
					$query['orderby']  = 'meta_value_num';
					$query['meta_key'] = '_price';// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
					break;
				case 'rating':
					$query['orderby']  = 'meta_value_num';
					$query['meta_key'] = '_wc_average_rating';// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
					break;
				default:
	...
	private static function get_query_product( $show_variation = false, $product_parents = array(), $product_exclude = array(), $cats_include = array(), $cats_exclude = array(), $product_price_min = '', $product_price_max = '' ) {
		...
		if ( $show_variation ) {
			...
			$args['post__not_in']    = $product_exclude;// phpcs:ignore WordPressVIPMinimum.Performance.WPQueryParams.PostNotIn_post__not_in
		} else {
			if ( ! empty( $product_parents ) ) {
				$args['post_type'] = array( 'product', 'product_variation' );
			}
